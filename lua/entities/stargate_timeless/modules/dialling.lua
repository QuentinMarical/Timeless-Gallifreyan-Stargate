	...
	if(inbound) then
		if (inbound and not fast and IsValid(t) and t.IsNewSlowDial) then
			...
			for i=1,chevs do
				...
				action:Add({f=self.SetWire,v={self,"Adresse de composition",DialAddress},d=0}); -- Wire
				action:Add({f=self.SetWire,v={self,"Symbole composé",DialSymbol},d=0}); -- Wire
				...
				if(i == chevs) then
					action:Add({f=self.SetWire,v={self,"Chevron verrouillé",1},d=0}); -- Wire
		...
		if (self.DialledAddress[1] != nil) then
			action:Add({f=self.SetWire,v={self,"Symbole de composition",tostring(self.DialledAddress[1])},d=0}); -- Wire
		end
		-- Start spinning
		...
		for i=1,chevs do
			...
			if(i < chevs or i == chevs and (not fail or busy) and not self.chev_destroyed[7]) then
				...
				action:Add({f=self.SetWire,v={self,"Adresse de composition",DialAddress},d=0}); -- Wire
				action:Add({f=self.SetWire,v={self,"Symbole composé",DialSymbol},d=0}); -- Wire
				if (i == chevs) then action:Add({f=self.SetWire,v={self,"Symbole de composition",""},d=0}); end -- Wire
				action:Add({f=self.EmitSound,v={self.Entity,snd,90,math.random(98,103)},d=0});
				action:Add({f=self.DHDSetChevron,v={self,i,0.05},d=0});
				local s = i;
				local b = i+1;
				...
				action:Add({f=self.SetWire,v={self,"Symbole de composition",DialNextSymbol},d=0}); -- Wire
				if(i == chevs) then
...
function ENT.Sequence:DialFail(instant_stop,play_sound)
	...
	action:Add({f=self.SetWire,v={self,"Adresse de composition",""},d=0}); -- Wire
	action:Add({f=self.SetWire,v={self,"Symbole de composition",""},d=0}); -- Wire
	action:Add({f=self.SetWire,v={self,"Symbole composé",""},d=0}); -- Wire
	action:Add({f=self.SetWire,v={self,"Chevron verrouillé",0},d=0}); -- Wire
	action:Add({f=self.SetChevrons,v={self,0,0},d=0}); -- Wire
...
function ENT.Sequence:InstantOpen(action,delay,instant,inbound,slow,nox,fail)
	...
	if (inbound) then
		action:Add({f=self.SetWire,v={self,"Entrant",1},d=0});
	end
...
function ENT.Sequence:OnButtonChevron(lightup, dialchev, address, symbol, fail, busy, slow)
	...
	if lightup then
		...
		action:Add({f=self.RingLight,v={self,0,true,true},d=0});
		action:Add({f=self.RingLight,v={self,0,false,true},d=0});
		action:Add({f=self.SetWire,v={self,"Symbole de composition",dialsymbol},d=0}); -- Wire
		--action:Add({f=self.EmitSound,v={self.Entity,self.Sounds.Ring,90,math.random(98,103)},d=0}); -- Ring Sound
